...
def station_1():
    ...
    def information_stream_Station1():
        ...
        path_Station1 = os.listdir("data/Station_1")
        # calulate number of audio files 
        n=len(path_Station1)
        # store them in array (fetching next song decision)
        path_Station1 = np.array(path_Station1)

        # TinyTag method is used extract the metadata
        # for loop used to send all audio files sequentially
        for x in range(len(path_Station1)):
            ...
            filesize = i.filesize
            duration = i.duration
            # if is to check whether next song exists or not
            ...
            # duration changes hence appended in while loop
            while(duration>0):
                ...
                # sleep used to send data every second
                time.sleep(1)
                duration = duration-1  
    ...
    # Threading used since this processes need to run together
...
# TCP connection for sending initial greetings and information
def tcp_connection():
    # This function is to establish handshake from client and send the radio list
    ...
    # This while lop starts a thread of each new connected client
    while True:
        c, addr = s.accept()  # Establish connection with client.
        t1=threading.Thread(target=on_new_client,args=(c, addr, pickled_radio_stn_info))
        t1.start()

# Three consecutive threads of TCP and Radio Stations are created
